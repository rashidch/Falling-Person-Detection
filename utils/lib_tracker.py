...
class Tracker(object):
    ...
    def track(self, curr_skels):
        ...
        num_humans_to_add = min(
            len(unmatched_idx), self._max_humans - len(good_matches)
        )
    ...
    def _match_features(self, features1, features2):
        ...
        def cost(sk1, sk2):
            ...
            joints = np.array(
                [2, 3, 4, 5, 6, 7, 10, 11, 12, 13, 16, 17, 18, 19, 22, 23, 24, 25]
            )
        ...
        if n1 and n2:
            ...
            for i1, i2 in enumerate(matches_f1_to_f2):
                if (
                    matches_f2_to_f1[i2] == i1
                    and dist_matrix[i1, i2] < self._dist_thresh
                ):
                    good_matches[i2] = i1
